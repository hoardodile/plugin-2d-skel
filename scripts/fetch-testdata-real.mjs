...
const SOURCES = [
	{
		...
		sourceUrl: "https://github.com/Speykious/arch-chan/tree/ed43dcf7e88d56f79d2e42fecb084bf6923f44e0",
		...
		extract(entries) {
			...
			for (const [name, bytes] of Object.entries(entries)) {
				...
				if (file === "" || file.endsWith("/") || file.startsWith("cmo3/")) continue
	...
	},
	{
		...
		license: "MIT (the repo is MIT; the sample is used for local testing only).",
		expected: Object.freeze({
		...
		extract(entries) {
			...
			for (const [name, bytes] of Object.entries(entries)) {
				const rel = name.slice(name.indexOf("/") + 1)
				const match = /\/(starter_ske\.json|starter_tex\.json|starter_tex\.png)$/.exec(rel)
...
async function fetchSource(source) {
	...
	for (const [rel, expected] of Object.entries(source.expected)) {
		...
		if (actual !== expected) {
			throw new Error(`${source.name}: extracted file sha256 mismatch: ${rel} (got ${actual})`)
		}
	}
	return written
}
...
function renderReadme() {
	...
	for (const source of SOURCES) {
		...
		notes.push(
			...
			`sha256:`,
			...Object.entries(source.expected).map(([rel, hash]) => `- ${rel}: ${hash}`),
			"",
		)
	}
	notes.push("Regenerated by `node scripts/fetch-testdata-real.mjs` (or `pnpm install`).", "")
...
async function main() {
	...
	if (pending.length === 0) {
		console.log("[testdata-real] present and verified — skipping (use --force to re-fetch).")
		return
	}
