...
describe("wheelScaleFromDelta", () => {
	test("zooms in on a negative delta and out on a positive one", () => {
		expect(
			wheelScaleFromDelta(1, -100, { minScale: 0.25, maxScale: 8 }),
		).toBeGreaterThan(1)
		expect(
			wheelScaleFromDelta(1, 100, { minScale: 0.25, maxScale: 8 }),
		).toBeLessThan(1)
	...
	test("clamps to the zoom range", () => {
		expect(
			wheelScaleFromDelta(0.3, 1000, { minScale: 0.25, maxScale: 8 }),
		).toBe(0.25)
		expect(wheelScaleFromDelta(7, -1000, { minScale: 0.25, maxScale: 8 })).toBe(
			8,
		)
	})
})
...
describe("clampPan", () => {
	...
	test("keeps the model center within the canvas interior", () => {
		expect(
			clampPan({ x: 5000, y: -5000, scale: 2, rotation: 0 }, view, 0.6),
		).toEqual({
			x: 600,
			y: -480,
			scale: 2,
	...
	test("leaves in-range transforms untouched (rotation carried through)", () => {
		expect(
			clampPan({ x: 10, y: 20, scale: 1, rotation: 0.5 }, view, 0.6),
		).toEqual({
			x: 10,
			y: 20,
			scale: 1,
	...
	test("scales the bound with the pan extent", () => {
		expect(
			clampPan({ x: 300, y: 300, scale: 1, rotation: 0 }, view, 0.25),
		).toEqual({
			x: 250,
			y: 200,
			scale: 1,
...
describe("clampZoomTransform", () => {
	...
	test("clamps an out-of-range scale", () => {
		expect(
			clampZoomTransform({ x: 0, y: 0, scale: 50, rotation: 0 }, view, options),
		).toHaveProperty("scale", 8)
		expect(
			clampZoomTransform(
				{ x: 0, y: 0, scale: 0.01, rotation: 0 },
				view,
				options,
			),
		).toHaveProperty("scale", 0.25)
	})

	test("keeps the pan reachable at the same time", () => {
		const next = clampZoomTransform(
			{ x: 5000, y: -5000, scale: 3, rotation: 0 },
			view,
			options,
		)
...
describe("pan / rotate / zoom carry rotation", () => {
	...
	test("panViewport adds the delta and keeps rotation", () => {
		expect(panViewport(t, { x: 5, y: -3 })).toEqual({
			x: 15,
			y: 17,
			scale: 2,
			rotation: 0.4,
	...
	test("rotateViewport adds only the radian delta", () => {
		expect(rotateViewport(t, 0.3)).toEqual({
			x: 10,
			y: 20,
			scale: 2,
			rotation: 0.7,
	...
	test("zoomViewportAt keeps rotation", () => {
		const next = zoomViewportAt(t, { x: 0, y: 0 }, 4, {
			minScale: 0.25,
			maxScale: 8,
		})
...
describe("deriveLive2dViewport", () => {
	...
	function applied(transform: {
		...
		const base =
			Math.min(
				screen.width / modelSize.width,
				screen.height / modelSize.height,
			) * 0.9
		return {
			position: {
	...
	test("mirror flips scale.x sign without changing the reported zoom magnitude", () => {
		...
		expect(deriveLive2dViewport(model, screen, modelSize, fitMode).scale).toBe(
			1,
		)
	})
	...
	test("returns home when the model has no usable size", () => {
		expect(
			deriveLive2dViewport(
				applied({ x: 10, y: 10, scale: 2, rotation: 0 }),
				screen,
				{ width: 0, height: 0 },
				fitMode,
			),
